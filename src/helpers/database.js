...
class DatabaseManager {
  ...
  createTables() {
    ...
    // 创建剪贴板翻译历史表
    this.db.exec(`
      CREATE TABLE IF NOT EXISTS translated_clipboard_history (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  saveTranslatedClipboard(original_text, translated_text) {
    if (!original_text || !translated_text) {
      throw new Error('原文和译文不能为空');
    }
    const stmt = this.db.prepare(`
      INSERT INTO translated_clipboard_history (
        original_text, translated_text
      ) VALUES (?, ?)
    `);
    return stmt.run(original_text.trim(), translated_text.trim());
  }

  getTranslatedClipboardHistory(limit = 50, offset = 0) {
    const stmt = this.db.prepare(`
      SELECT * FROM translated_clipboard_history 
      ORDER BY created_at DESC 
      LIMIT ? OFFSET ?
    `);
    return stmt.all(limit, offset);
  ...
  deleteTranslatedClipboardItem(id) {
    const stmt = this.db.prepare("DELETE FROM translated_clipboard_history WHERE id = ?");
    return stmt.run(id);
  }

  clearTranslatedClipboardHistory() {
    const stmt = this.db.prepare("DELETE FROM translated_clipboard_history");
    return stmt.run();
  }
